...
function Postgres(a, b) {
  ...
  function transaction({
    ...
    scoped.savepoint = (name, fn) => new Promise((resolve, reject) => {
      transaction({
        savepoint: 'savepoint ' + escape('s' + connection.savepoints++ + '_' + (fn ? name : '')),
        resolve,
        reject,
        fn: fn || name
      }, connection)
    ...
    query({ raw: true }, connection, begin || savepoint)
      ...
      .catch((err) => {
        ...
        .then(() => reject(err))
        .catch((rollbackErr) => reject(rollbackErr))
      })
      .then(begin && (() => {
  ...
  function next() {
    ...
    while (queries.length && (c = getConnection(queries.peek().fn || queries.peek().cursor)) && (x = queries.shift())) {
      x.fn
        ? transaction(x, c)
  ...
  function query(query, connection, xs, args) {
    query.timers = {}
    query.timers.scheduled_at = performance.now()
    query.origin = options.debug ? new Error().stack : cachedError(xs)
    query.dynamic = query.dynamic || (options.no_prepare && !query.overwriteNoPrepare)
    ...
    addMethods(promise, query, connection)
  ...
  function getConnection(reserve) {
    ...
    if (reserve && connection) connection.reserved = true
    return connection
  }
  ...
  function addTypes(sql, connection) {
    ...
    function noPrepare(xs) {
      return query({ dynamic: true }, connection || getConnection(), xs, Array.from(arguments).slice(1))
    }
    ...
    function file(path, args, options = {}) {
      ...
      addMethods(promise, q, connection)
  ...
  function addMethods(promise, query, connection) {
    ...
    promise.cursor = (rows, fn) => {
      ...
      // prevent connection with running cursor query from being used concurrently by another query
      if (connection) {
      ...
      return promise
        .finally(() => {
          // return reserved connection back to the pool
          if (connection) {
            connection.reserved = false
            connections.push(connection)
          }
  ...
  function parseArray(array, xargs, types) {
    return array.length === 0 ? '\'{}\'' : 'array[' + array.map((x) => Array.isArray(x)
      ...
      : parseValue(
          // We must pass explicit correct number type of array element instead of 0,
          //  because apparently postgres auto-detecting doesn't work for array elements.
          // Preventing "column is of type integer[] but expression is of type text[]" error.
          typeof x === 'number'
            ? { type: inferNumberType(x), value: x }
            : x,
          xargs,
          types
        )
