...
function Connection(options = {}) {
  ...
  function error(err) {
    ...
    // connection closing is asyncronous and
    // while socket close is performed new queries may be scheduled to this connection
    // so we shouldn't reject those queries if connection close is caused by idle timeout
    if (!idleClosing) {
      let q
  ...
  function closeOnIdle() {
    idleClosing = true
    // we must immediately unset these flags without waiting for actual async connection close callback
    // so any concurrently incomming queries may be correctly delayed until connection is reopened
    open = ready = false
    socket.end()
  }
