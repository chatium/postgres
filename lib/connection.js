const net = require('net')
const tls = require('tls')
const frontend = require('./frontend.js')
const Backend = require('./backend.js')
const Queue = require('./queue.js')
const { END } = require('./types.js')
const { errors } = require('./errors.js')
const { performance } = require('perf_hooks')

module.exports = Connection

let count = 1

function Connection(options = {}) {
  const {
    onparameter,
    transform,
    idle_timeout,
    connect_timeout,
    onnotify,
    onnotice,
    onclose,
    parsers
  } = options
  let buffer = Buffer.alloc(0)
  let length = 0
  let messages = []
  let timer
  // indicates that current connection closing is caused by the idle timeout and query queue shouldn't be rejected
  let idleClosing = false
  let statement_id = 1
  let ended
  let open = false
  let ready = false
  let statements = {}
  let connect_timer

  const queries = Queue()
      , id = count++
      , uid = Math.random().toString(36).slice(2)
      , connection = { send, end, destroy, reserved: false }

  const socket = postgresSocket(options, {
    ready,
    data,
    error,
    close
  })

  const backend = Backend({
    onparse,
    onparameter,
    onsuspended,
    oncomplete,
    transform,
    parsers,
    onnotify,
    onnotice,
    onready,
    onauth,
    error
  })

  function onsuspended(x) {
    new Promise(r => r(backend.query.cursor(
      backend.query.cursor.rows === 1 ? x[0] : x
    ))).then(x => {
      x === END
        ? socket.write(frontend.Close())
        : socket.write(frontend.Execute(backend.query.cursor.rows))
    }).catch(err => {
      rejectQuery(backend.query, err)
      socket.write(frontend.Close())
    })
  }

  function oncomplete(result) {
    if (backend.query.cursor) {
      if (result.length) {
        // emit final chunk to cursor handler
        new Promise(r => r(backend.query.cursor(
          backend.query.cursor.rows === 1 ? result[0] : result
        ))).then(() => {
          socket.write(frontend.Close())
        }).catch(err => {
          rejectQuery(backend.query, err)
          socket.write(frontend.Close())
        })
      } else {
        socket.write(frontend.Close())
      }
    }
  }

  function onparse() {
    if (backend.query && backend.query.statement.sig)
      statements[backend.query.statement.sig] = backend.query.statement
  }

  function onauth(type, x, onerror) {
    Promise.resolve(
      typeof options.pass === 'function'
        ? options.pass()
        : options.pass
    ).then(pass =>
      socket.write(frontend.auth(type, x, options, pass))
    ).catch(onerror)
  }

  function end() {
    clearTimeout(timer)
    const promise = new Promise((resolve) => {
      ended = () => resolve(socket.end())
    })

    process.nextTick(() => (ready || !backend.query) && ended())

    return promise
  }

  function destroy() {
    error(errors.connection('CONNECTION_DESTROYED', options))
    socket.destroy()
  }

  function error(err) {
    backend.query && rejectQuery(backend.query, err)
    let q
    while ((q = queries.shift()))
      rejectQuery(q, err)
  }

  function send(query, { sig, str, args = [] }) {
    try {
      query.str = str
      query.args = args
      query.result = []
      query.result.count = null
      idle_timeout && clearTimeout(timer)

      typeof options.debug === 'function' && options.debug(id, str, args)
      const buffer = query.simple
        ? simple(str, query)
        : sig in statements
          ? prepared(statements[sig], args, query)
          : prepare(sig, str, args, query)

      ready
        ? (backend.query = query, query.timers && (query.timers.executed_at = performance.now()), ready = false)
        : queries.push(query)

      open
        ? (
            query.timers && (query.timers.socket_write_started_at = performance.now()),
            socket.write(buffer, () => {
              query.timers && (query.timers.socket_write_finished_at = performance.now())
            })
        )
        : (messages.push({buffer, query}), connect())
    } catch (err) {
      rejectQuery(query, err)
      idle()
    }
  }

  function connect() {
    connect_timeout && (
      clearTimeout(connect_timer),
      connect_timer = setTimeout(connectTimedOut, connect_timeout * 1000)
    )
    // resolve racing between concurrent idle connection closing and new queries sent
    socket.closing
      ? socket.closing.then(() => socket.connect())
      : socket.connect()
  }

  function connectTimedOut() {
    error(errors.connection('CONNECT_TIMEOUT', options))
    socket.destroy()
  }

  function simple(str, query) {
    query.statement = {}
    return frontend.Query(str)
  }

  function prepared(statement, args, query) {
    query.statement = statement
    return bind(query, args)
  }

  function prepare(sig, str, args, query) {
    query.statement = { name: sig ? 'p' + uid + statement_id++ : '', sig }
    return Buffer.concat([
      frontend.Parse(query.statement.name, str, args),
      bind(query, args)
    ])
  }

  function bind(query, args) {
    return query.cursor
      ? frontend.Bind(query.statement.name, args, query.cursor.rows)
      : frontend.Bind(query.statement.name, args)
  }

  function idle() {
    if (idle_timeout && !backend.query && queries.length === 0) {
      clearTimeout(timer)
      timer = setTimeout(closeOnIdle, idle_timeout * 1000)
    }
  }

  function closeOnIdle() {
    // if connection reserved - it should not be closed
    if (connection.reserved) return
    // this flag tells connection close callback to not reset the following state variables again
    idleClosing = true
    // we must immediately reset this state variables without waiting for actual async connection close callback
    // so any new queries may be correctly scheduled until connection is reopened
    // see close() function
    statements = {}
    // this is probably redundant, messages should be empty, but leaving here just in case for consistency
    messages = []
    open = ready = false
    socket.end()
  }

  function rejectQuery(query, err) {
    if (query.origin) {
      err.stack += query.origin.replace(/.*\n/, '\n')
    }
    if (!err.query) {
      Object.defineProperty(err, 'query', {
        value: query.str,
        enumerable: !!options.debug
      })
    }
    if (!err.parameters) {
      Object.defineProperty(err, 'parameters', {
        value: query.args,
        enumerable: !!options.debug
      })
    }
    query.reject(err)
  }

  function onready(err) {
    clearTimeout(connect_timer)
    if (err) {
      if (backend.query) {
        rejectQuery(backend.query, err)
      } else {
        error(err)
      }
    } else if (backend.query) {
      // resolve to tuple
      const resolveTo = backend.query.results || backend.query.result
      if (resolveTo && typeof resolveTo === 'object') {
        Object.defineProperty(resolveTo, '___QUERY___', {
          enumerable: false,
          value: backend.query
        })
        Object.defineProperty(resolveTo, '___TIMERS___', {
          enumerable: false,
          value: backend.query.timers,
        })
      }
      if (backend.query.timers) {
        backend.query.timers.finished_at = performance.now()
      }
      backend.query.resolve(resolveTo)
    }

    backend.query = backend.error = null
    idle()

    if (!open) {
      messages.forEach(({buffer, query}) => {
        if (query.timers) {
          query.timers.socket_write_started_at = performance.now()
        }
        socket.write(buffer, () => {
          if (query.timers) {
            query.timers.socket_write_finished_at = performance.now()
          }
        })
      })
      messages = []
      open = true
    }

    backend.query = queries.shift()
    if (backend.query && backend.query.timers) {
      backend.query.timers.executed_at = performance.now()
    }
    ready = !backend.query
    ready && ended && ended()
  }

  function data(x) {
    buffer = buffer.length === 0
      ? x
      : Buffer.concat([buffer, x], buffer.length + x.length)

    while (buffer.length > 4) {
      length = buffer.readInt32BE(1)
      if (length >= buffer.length)
        break

      backend[buffer[0]](buffer.slice(0, length + 1))
      buffer = buffer.slice(length + 1)
    }
  }

  function close() {
    clearTimeout(connect_timer)
    // if this close is caused by idle timeout
    // then this state reset has already been done (see closeOnIdle() fn)
    // and shouldn't be done here,
    // because new queries may already concurrently scheduled their messages,
    // while waiting for async connection closing process
    if (!idleClosing) {
      // we aslo call this error only when connection close is not caused by the idle timeout
      // because that means that means that backend.query and queries are empty (by definition of idle connection)
      // and this isn't really an error
      error(errors.connection('CONNECTION_CLOSED', options))
      statements = {}
      messages = []
      open = ready = false
    }
    if (idle_timeout) {
      clearTimeout(timer)
      idleClosing = false
    }
    onclose && onclose()
  }

  /* c8 ignore next */
  return connection
}

function postgresSocket(options, {
  error,
  close,
  data,
}) {
  let socket
  let closed = true
  let next = null
  let buffer
  let onWriteCallbacks = []

  function onclose() {
    socket.removeListener('data', data)
    socket.removeListener('error', error)
    socket.removeListener('connect', ready)
    socket.removeListener('secureConnect', ready)
    closed = true
    close()
  }

  function connect() {
    if (!closed)
      return

    closed = false
    x.closing = false

    const socket = options.path
      ? net.connect(options.path)
      : net.connect(options.port, options.host)

    if (typeof options.onsocket === 'function') {
      options.onsocket(socket)
    }

    if (!options.ssl)
      return attach(socket)

    socket.once('connect', () => socket.write(Buffer.from('0000000804d2162f', 'hex')))
    socket.once('error', error)
    socket.once('close', onclose)
    socket.once('data', x => {
      socket.removeListener('error', error)
      socket.removeListener('close', onclose)
      x.toString() === 'S'
        ? attach(tls.connect(Object.assign({ socket }, options.ssl)))
        : /* c8 ignore next */ error('Server does not support SSL')
    })
  }

  function attach(x) {
    socket = x
    socket.on('data', data)
    socket.once('error', error)
    socket.once('connect', ready)
    socket.once('secureConnect', ready)
    socket.once('close', onclose)
  }

  function ready() {
    try {
      socket.write(frontend.connect(options))
    } catch (e) {
      error(e)
      socket.end()
    }
  }

  const x = {
    write: (x, cb) => {
      buffer = buffer ? Buffer.concat([buffer, x]) : Buffer.from(x)
      if (typeof cb === 'function') {
        onWriteCallbacks.push(cb)
      }
      if (buffer.length >= 1024)
        return write()
      next === null && (next = setImmediate(write))
    },
    destroy: () => {
      socket && socket.destroy()
      return Promise.resolve()
    },
    end: () => {
      x.closing = new Promise(r => socket && !closed ? (socket.once('close', r), socket.end()) : r())
      return x.closing
    },
    connect,
    // allows connection.connect() to wait until current socket closing process is finished
    closing: false,
  }

  function write() {
    const thisWriteCallbacks = onWriteCallbacks
    onWriteCallbacks = []
    socket.write(buffer, () => {
      thisWriteCallbacks.forEach(cb => cb());
    })
    next !== null && clearImmediate(next)
    buffer = next = null
  }

  /* c8 ignore next */
  return x
}
