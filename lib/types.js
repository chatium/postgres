...
module.exports.escape = function escape(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (str[i] === '.' && str.slice(0, i).toLowerCase() === 'excluded') {
      // allow safely create ON CONFLICT DO UPDATE SET clauses with special 'excluded' table name supported by postgres
      unquotedPrefix = 'EXCLUDED.'
      last = i + 1
    } else if (str[i] === '"') {
      q = true
  ...
  return unquotedPrefix +
    (q ? '"' : '') +
    (q ? result + str.slice(last, str.length) : unquotedPrefix ? str.slice(unquotedPrefix.length) : str) +
    (q ? '"' : '')
}
...
module.exports.inferNumberType = function inferNumberType(x) {
